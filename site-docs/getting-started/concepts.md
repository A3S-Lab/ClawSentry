...
L1 引擎内置三条短路规则，命中时立即产生判决，不进行完整的五维评分：
...
## 5. D1-D5 五维风险评分 {#risk-dimensions}

ClawSentry 通过五个维度对每个事件进行定量风险评估，生成不可变的 `RiskSnapshot`。
...
$$
\text{composite\_score} = \max(D1, D2, D3) + D4 + D5 \quad (\text{范围: } 0\text{-}7)
$$
