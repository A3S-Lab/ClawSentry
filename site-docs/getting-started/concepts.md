...
| 规则 | 条件 | 判决 |
|------|------|------|
| **SC-1** | 检测到已知危险命令模式（如 `rm -rf /`、`:(){ :\|:& };:`） | 立即 BLOCK |
| **SC-2** | 工具为只读类型（如 `read_file`、`grep`） | 立即 ALLOW |
| **SC-3** | 检测到凭证/密钥泄露模式 | 立即 BLOCK |
...
!!! example "D6 放大效应示例"
    假设一个 `pip install` 命令的 base_score = 0.95（MEDIUM 区间）：

    | 场景 | D6 分值 | 乘数 | composite_score | 最终判决 |
    |------|:-------:|:----:|:---------------:|:-------:|
    | 无注入特征 | 0.0 | 1.00 | 0.95 | MEDIUM → L2 分析 |
    | 含 2 条弱注入模式（+0.6） | 0.6 | 1.10 | 1.05 | HIGH → BLOCK |
    | 强注入 + Canary 泄露（+2.3） | 2.3 | 1.38 | 1.31 | HIGH → BLOCK |
    | Layer 3 向量命中 + 强模式（3.0） | 3.0 | 1.50 | 1.43 | HIGH → BLOCK |

    D6 将一个本来只触发 L2 分析的 MEDIUM 操作直接拦截为 HIGH。
