...
ClawSentry 使用五个独立维度对事件进行风险评估，每个维度产生一个整数分值，最终组合为综合风险分数。

### 评分公式

```
composite_score = max(D1, D2, D3) + D4 + D5
```
...
=== "安全读取"
    ...
    D1=0 (只读工具)   D2=0 (普通路径)   D3=0 (非bash)   D4=0 (无高危)   D5=1 (标准)
    composite_score = max(0,0,0) + 0 + 1 = 1
    风险等级: low → ALLOW

    短路规则 SC-3 命中: D1=0, D2=0, D3=0 → 直接判定 low
    ```
...
=== "删除系统文件"
    ...
    D1=3 (高危命令)   D2=3 (系统路径)   D3=3 (rm -rf)   D4=0 (<2次)   D5=2 (不信任)
    composite_score = max(3,3,3) + 0 + 2 = 5
    风险等级: critical → BLOCK

    短路规则 SC-1 命中: D1=3, D2=3(>=2) → 直接判定 critical
    短路规则 SC-2 命中: D3=3 → 直接判定 critical
    ```
...
=== "安装包"
    ...
    D1=2 (bash工具)   D2=1 (默认)   D3=2 (pip install)   D4=0 (无高危)   D5=1 (标准)
    composite_score = max(2,1,2) + 0 + 1 = 3
