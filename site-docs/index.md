...
**AI Agent 会执行危险命令。ClawSentry 在可拦截框架中前置阻断，在 Codex 中实时监控告警。**
{ .tagline }
...
### :shield: 拦截优先，监控兜底
Claude Code、a3s-code 和 OpenClaw 可在高危操作前阻断；Codex 通过 session 日志提供实时风险评估和告警。
</div>
...
通过显式 SDK stdio / HTTP Transport 接入，**自动拦截**高危操作。
...
??? info "各框架接入详情"
    ...
    === "Codex CLI"
        ...
        通过 **Session 日志监控**接入，不修改 Codex 配置，也不会自动拦截操作。
