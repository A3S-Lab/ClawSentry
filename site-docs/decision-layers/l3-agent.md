...
| 条件 | 判断逻辑 | 说明 |
|------|----------|------|
| **手动触发标志** | `DecisionContext` 含 `l3_escalate`/`force_l3`/`manual_l3_escalation` | 运维人员或 API 显式请求 L3 审查 |
...
| ReadOnlyToolkit | `src/clawsentry/gateway/review_toolkit.py` | 5 个只读工具 + 路径沙箱 + 调用预算 |
