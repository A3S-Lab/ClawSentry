...
| 层级 | 名称 | 机制 | 分值贡献 |
|:----:|------|------|---------|
| **Layer 1** | 启发式正则 | 弱模式（10 条，+0.3/条，上限 1.5）<br>强模式（8 条，+0.8/条，上限 2.4）<br>工具特定模式（+0.5/条，上限 1.0） | 0.0–3.0 |
...
=== "强模式（+0.8 each）"
    ...
    - `<script` 标签（XSS 注入）
    - `data:text/html;base64,`（Base64 编码页面）
    - `eval(` / `exec(` / `__import__(` （代码执行）
    - `<!--[^>]*ignore`（HTML 注释隐藏注入）
    - Unicode 零宽字符 `\u200b \u200c \u200d \ufeff`（不可见字符注入）
    - `data:...base64,...curl/wget`（Base64 编码的网络请求）
...
**D6 乘数对 base_score 的放大效果示例：**

| D6 值 | 乘数公式 `1.0 + 0.5×(D6/3.0)` | 乘数 | base=0.95 时 composite | 风险等级变化 |
|:-----:|-------------------------------|:----:|:---------------------:|:-----------:|
| 0.0 | 1.0 + 0.5 × 0.0 | **1.00** | 0.95 | MEDIUM（触发 L2） |
| 1.5 | 1.0 + 0.5 × 0.5 | **1.25** | 1.19 | MEDIUM（触发 L2） |
| 2.0 | 1.0 + 0.5 × 0.67 | **1.33** | 1.26 | HIGH → BLOCK |
| 3.0 | 1.0 + 0.5 × 1.0 | **1.50** | 1.43 | HIGH → BLOCK |

> D6 将一个原本只进入 L2 分析的 MEDIUM 操作，直接拦截为 HIGH。
